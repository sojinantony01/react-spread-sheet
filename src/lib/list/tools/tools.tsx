...
const SINGLE_FORMULAS = [
  { name: "ABS",   template: "ABS(A1)",       desc: "Absolute (positive) value of a cell — e.g. ABS(-5) = 5" },
  { name: "SQRT",  template: "SQRT(A1)",      desc: "Square root of a cell value" },
  { name: "ROUND", template: "ROUND(A1,2)",   desc: "Round a cell to N decimal places — e.g. ROUND(A1,2)" },
  { name: "POWER", template: "POWER(A1,2)",   desc: "Raise a cell value to a power — e.g. POWER(A1,3) = A1³" },
  { name: "IF",    template: "IF(A1>0,A1,0)", desc: "If condition is true return one value, else another" },
];
...
const Tools = ({
  ...
  const onValChange = (e: { target: { value: string } }) => {
    ...
    if (val.startsWith("=") && i !== undefined && j !== undefined) {
      ...
      updateFormulaHighlights(val, i, j, headerValues, state.data.length, state.data[0]?.length ?? 0);
    } else {
    ...
    (formulaName: string) => {
      ...
      if (isSingleCell) {
        ...
        if (sel.length > 1) {
          ...
          rangeStr = buildRangeString(firstCell[0], firstCell[1], lastCell[0], lastCell[1], headerValues);
      ...
      <div className="sheet-tools">
          ...
          {showFormulaMenu && (
            <div className="sheet-formula-toolbar-menu" data-testid="formula-toolbar-menu">
              <div className="sheet-formula-toolbar-header">
                {selectedCount > 1 ? (
                  <span className="fx-range-hint">{selectedCount} cells selected — result placed below</span>
                ) : (
        ...
        <div className="sheet-tools-text-style-container">
          <button
            ...
            onClick={() => { dispatch(undo); onChange && onChange(); }}
          >
          ...
          <button
            ...
            onClick={() => { dispatch(redo); onChange && onChange(); }}
          >
            <svg width={15} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 7H9.00001C7.13077 7 6.19615 7 5.5 7.40193C5.04395 7.66523 4.66524 8.04394 4.40193 8.49999C4 9.19615 4 10.1308 4 12C4 13.8692 4 14.8038 4.40192 15.5C4.66523 15.9561 5.04394 16.3348 5.5 16.5981C6.19615 17 7.13077 17 9 17H16M20 7L17 4M20 7L17 10"
                stroke="#1C274C" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        </div>
        ...
        <div className="sheet-tools-font-size-container">
          <button data-testid="font-size-decrease"
          ...
          <button data-testid="font-size-increase"
            onClick={() => changeStyle("FONT", (parseInt(selectedFontSize) + 1).toString())}>
            <svg width={10} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
        ...
        <div className="sheet-tools-text-cell-color-container">
          <button data-testid="font-color-button" onClick={() => fontColorRef.current?.click()}>
            A{" "}
            <span className="sheet-color-strip"
              style={{ backgroundColor: parsedSelectedStyles["color"] }}></span>
          </button>
          <input ref={fontColorRef} type="color" data-testid="font-color"
            value={parsedSelectedStyles?.["color"] || "#000000"}
            onChange={(e) => changeStyleWithDebounce("COLOR", e.target.value)} />
          <button data-testid="background-color-button"
            onClick={() => backgroundColorRef.current?.click()}>
            <svg fill={parsedSelectedStyles?.["background"]} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
            ...
            <span className="sheet-color-strip"
              style={{ backgroundColor: parsedSelectedStyles["background"] }}></span>
          </button>
          <input ref={backgroundColorRef} type="color" data-testid="background-color"
        ...
        <div className="sheet-tools-text-style-container">
          <button
            ...
            onClick={() => changeStyle("B")}>B</button>
          <button
            className={parsedSelectedStyles["text-decoration"] === "underline"
              ? "text-style-btn-active text-style-btn-active-underline" : ""}
            onClick={() => changeStyle("U")}>U</button>
          <button
            className={parsedSelectedStyles["fontStyle"] === "italic" ? "text-style-btn-active" : ""}
            onClick={() => changeStyle("I")}>I</button>
        </div>

        {/* Text alignment */}
        <div className="sheet-tools-text-align-container">
          <button className={parsedSelectedStyles["textAlign"] === "left" ? "text-style-btn-active" : ""}
            data-testid="align-left" onClick={() => changeStyle("ALIGN-LEFT")}>
            <Icons type="align-left" /></button>
          <button className={parsedSelectedStyles["textAlign"] === "center" ? "text-style-btn-active" : ""}
            data-testid="align-center" onClick={() => changeStyle("ALIGN-CENTER")}>
            <Icons type="align-center" /></button>
          <button className={parsedSelectedStyles["textAlign"] === "right" ? "text-style-btn-active" : ""}
            data-testid="align-right" onClick={() => changeStyle("ALIGN-RIGHT")}>
            <Icons type="align-right" /></button>
          <button className={parsedSelectedStyles["textAlign"] === "justify" ? "text-style-btn-active" : ""}
            data-testid="align-justify" onClick={() => changeStyle("ALIGN-JUSTIFY")}>
            <Icons type="align-justify" /></button>
        </div>
        ...
        <div className="sheet-tools-text-style-container">
          <button
            ...
            data-testid="merge"
            onClick={() => { dispatch(mergeCells); onChange && onChange(); }}
          >
            <Icons type="merge" />
          </button>
        </div>
